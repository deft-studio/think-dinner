...
- **フロントエンド**：Next.js + shadcn/ui。既存商品と同じ構成にして、慣れている部分は流用しつつ、今回新しい部分（AI連携・DB）に集中する。画面デザインはVercelのv0・Magic Patterns等で作成し、生成されたコードを骨組みとして使いながら、自分の意図に合わせて直していく方針。v0の生成コードがshadcn/ui前提のため、shadcn/uiをそのまま採用した。
